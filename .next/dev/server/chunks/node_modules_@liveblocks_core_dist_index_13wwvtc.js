module.exports = [
"[project]/node_modules/@liveblocks/core/dist/index.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
${parentKey}`;
   ...
   */ getByParentId(parentId) {
    ...
    /** All still-unacknowledged ops, in dispatch order. */ values() {
...
function createManagedPool(options) {
    const { getCurrentConnectionId, onDispatch, isStorageWritable = ()=>true, unacknowledgedOps = new UnacknowledgedOps() } = options;
    let clock = 0;
    let opClock = 0;
...
var AbstractCrdt = class {
   ...
   */ _toOpsWithOpId(parentId, parentKey, pool) {
    ...
    /** This caches the result of the last .toJSON() call for this Live node. */ #cachedJson;
    #cachedTreeNodeKey;
...
function createBatchStore(batch2, options) {
    ...
    function enqueue(input) {
        ...
        const pendingEnqueue$ = processEnqueue(input, cacheKey, batchResult$, pendingEnqueueId);
...
async function uploadRoomFile({ file, signal, abortErrorMessage, retryMultipartCompletion, uploadSingle, createMultipartUpload, uploadMultipartPart, completeMultipartUpload, abortMultipartUpload }) {
    ...
    if (file.size <= ROOM_FILE_PART_SIZE) {
        return autoRetry(uploadSingle, ROOM_FILE_RETRY_ATTEMPTS, ROOM_FILE_RETRY_DELAYS, handleRetryError);
    }
    let uploadId;
    const uploadedParts = [];
    const multipartUpload = await autoRetry(createMultipartUpload, ROOM_FILE_RETRY_ATTEMPTS, ROOM_FILE_RETRY_DELAYS, handleRetryError);
    ...
    try {
        ...
        for (const parts of batches){
            ...
            for (const { part, partNumber } of parts){
                partUploads$.push(autoRetry(()=>uploadMultipartPart(multipartUpload.uploadId, partNumber, part, partUploadSignal), ROOM_FILE_RETRY_ATTEMPTS, ROOM_FILE_RETRY_DELAYS, (error3)=>{
                    if (signal?.aborted) {
                        throw abortError;
                    }
        ...
        return retryMultipartCompletion ? autoRetry(()=>completeMultipartUpload(multipartUpload.uploadId, sortedParts), ROOM_FILE_RETRY_ATTEMPTS, ROOM_FILE_RETRY_DELAYS, handleRetryError) : completeMultipartUpload(uploadId, sortedParts);
    } catch (error3) {
        if (uploadId) {
            try {
...
function createApiClient({ baseUrl, authManager, fetchPolyfill }) {
    ...
    async function uploadAttachment(options) {
        ...
        return uploadRoomFile({
            ...
            uploadSingle: async ()=>httpClient.putBlob(url`/v2/c/rooms/${roomId}/attachments/${attachment.id}/upload/${attachment.name}`, await authManager.getAuthValue({
                    roomId,
                    resource: "comments",
                    access: "write"
                }), attachment.file, {
            ...
            createMultipartUpload: async ()=>httpClient.post(url`/v2/c/rooms/${roomId}/attachments/${attachment.id}/multipart/${attachment.name}`, await authManager.getAuthValue({
                    roomId,
                    resource: "comments",
                    access: "write"
                }), void 0, {
            ...
            uploadMultipartPart: async (uploadId, partNumber, part, signal)=>httpClient.putBlob(url`/v2/c/rooms/${roomId}/attachments/${attachment.id}/multipart/${uploadId}/${String(partNumber)}`, await authManager.getAuthValue({
                    roomId,
                    resource: "comments",
                    access: "write"
                }), part, void 0, {
            ...
            completeMultipartUpload: async (uploadId, parts)=>httpClient.post(url`/v2/c/rooms/${roomId}/attachments/${attachment.id}/multipart/${uploadId}/complete`, await authManager.getAuthValue({
                    roomId,
                    resource: "comments",
                    access: "write"
                }), {
    ...
    async function uploadFile(options) {
        ...
        const fileData = await uploadRoomFile({
            ...
            uploadSingle: async ()=>httpClient.putBlob(url`/v2/c/rooms/${roomId}/storage/files/${fileId}/upload/${file.name}`, await authManager.getAuthValue({
                    roomId,
                    resource: "storage",
                    access: "write"
                }), file, {
            ...
            createMultipartUpload: async ()=>httpClient.post(url`/v2/c/rooms/${roomId}/storage/files/${fileId}/multipart/${file.name}`, await authManager.getAuthValue({
                    roomId,
                    resource: "storage",
                    access: "write"
                }), void 0, {
            ...
            uploadMultipartPart: async (uploadId, partNumber, part, signal)=>httpClient.putBlob(url`/v2/c/rooms/${roomId}/storage/files/${fileId}/multipart/${uploadId}/${String(partNumber)}`, await authManager.getAuthValue({
                    roomId,
                    resource: "storage",
                    access: "write"
                }), part, void 0, {
            ...
            completeMultipartUpload: async (uploadId, parts)=>httpClient.post(url`/v2/c/rooms/${roomId}/storage/files/${fileId}/multipart/${uploadId}/complete`, await authManager.getAuthValue({
                    roomId,
                    resource: "storage",
                    access: "write"
                }), {
            ...
            abortMultipartUpload: async (uploadId)=>{
                await httpClient.rawDelete(url`/v2/c/rooms/${roomId}/storage/files/${fileId}/multipart/${uploadId}`, await authManager.getAuthValue({
                    roomId,
                    resource: "storage",
                    access: "write"
                }));
    ...
    const fileUrlsBatchStoresByRoom = new DefaultMap((roomId)=>{
        ...
        const shouldRetryPendingFileUrl = (fileId)=>{
            ...
            pendingFileUrlAttempts.set(fileId, {
                count,
                cleanupTimeoutId: setTimeout(()=>pendingFileUrlAttempts.delete(fileId), FILE_URL_ERROR_ATTEMPTS_EXPIRY)
            });
            return true;
        };
        const batch2 = new Batch(async (batchedFileIds)=>{
            const fileIds = batchedFileIds.flat();
            const { urls, expiresAt } = await httpClient.post(url`/v2/c/rooms/${roomId}/storage/files/presigned-urls`, await authManager.getAuthValue({
                roomId,
                resource: "storage",
                access: "read"
            }), {
            ...
            return urls.map((url2, index)=>{
                ...
                if (url2 === false) {
                    return shouldRetryPendingFileUrl(fileId) ? new FileUrlUnavailableError("There was an error while getting this file's URL") : new Error("There was an error while getting this file's URL");
                }
    ...
    async function fetchStorageHistoryVersion(options) {
        return httpClient.rawGet(url`/v2/c/rooms/${options.roomId}/versions/${options.versionId}/storage`, await authManager.getAuthValue({
            roomId: options.roomId,
            resource: "storage",
            access: "read"
        }));
    ...
    async function fetchYjsHistoryVersion(options) {
        return httpClient.rawGet(url`/v2/c/rooms/${options.roomId}/versions/${options.versionId}/yjs`, await authManager.getAuthValue({
            roomId: options.roomId,
            resource: "storage",
            access: "read"
        }));
    ...
    async function createVersionHistorySnapshot(options) {
        await httpClient.rawPost(url`/v2/c/rooms/${options.roomId}/versions`, await authManager.getAuthValue({
            roomId: options.roomId,
            resource: "storage",
            access: "write"
        }));
    ...
    async function deleteHistoryVersion(options) {
        await httpClient.delete(url`/v2/c/rooms/${options.roomId}/versions/${options.versionId}`, await authManager.getAuthValue({
            roomId: options.roomId,
            resource: "storage",
            access: "write"
        }));
...
function enableTracing(machine) {
    ...
    const unsubs = [
        ...
        machine.events.didIgnoreUnexpectedEvent.subscribe((e)=>log2("Ignored unexpected event", e.type, e, "(no transition declared)"))
    ];
    return ()=>{
...
function permissionMatrixFromScopes(scopes) {
    return permissionMatrixFromExplicitPermissions(explicitPermissionMatrixFromScopes(scopes));
}
function hasPermissionAccess(matrix, resource, requiredAccess) {
    const requiredRank = ACCESS_LEVEL_RANKS[requiredAccess];
    if (resource === "comments") {
        const commentsRank = Math.max(ACCESS_LEVEL_RANKS[matrix.comments ?? "none"], ACCESS_LEVEL_RANKS[matrix["comments:public"] ?? "none"], ACCESS_LEVEL_RANKS[matrix["comments:private"] ?? "none"]);
...
function resolveRoomPermissionMatrix(permissions, roomId) {
    const matchedPermissions = permissions.filter((entry)=>roomPatternMatches(entry.pattern, roomId));
...
function normalizeRoomAccesses(accesses) {
    ...
    return Object.fromEntries(Object.entries(accesses).map(([id, permissions])=>[
            id,
...
function mergeRoomPermissionScopes({ defaultAccesses, groupsAccesses, userAccesses }) {
    const sources = [
        explicitPermissionMatrixFromScopes(defaultAccesses),
        mergeExplicitPermissionMatricesByHighestAccess(groupsAccesses.map(explicitPermissionMatrixFromScopes)),
    ...
    return permissionMatrixToScopes(permissionMatrixFromExplicitPermissions(merged));
}
...
function permissionForAccessLevel(resource, access, field = resource) {
    ...
    throw new Error(`Invalid permission level for ${field}: ${JSON.stringify(access) ?? String(access)}`);
}
...
function makeCachedToken(token, expiresAt) {
    if (token.parsed.k === "acc" /* ACCESS_TOKEN */ ) {
        return {
            token,
            expiresAt,
            permissions: Object.entries(token.parsed.perms).map(([pattern, scopes])=>({
                    pattern,
...
function cachedTokenSatisfiesRequest(cachedToken, request) {
    ...
    const matrix = resolveRoomPermissionMatrix(cachedToken.permissions ?? [], request.roomId);
    if (matrix === void 0) {
        return false;
    }
...
var LiveList = class _LiveList extends AbstractCrdt {
   ...
   */ _toOps(parentId, parentKey) {
        ...
        for (const item of this.#items){
            ...
            const childOps = addIntentToRootOp(item._toOps(this._id, parentKey2), "set");
   ...
   */ #unacknowledgedSetOpIdAt(position) {
        ...
        let opId;
        for (const op of this._pool.unacknowledgedOps.getByParentIdAndKey(this._id, position)){
            if (op.intent === "set") {
                opId = op.opId;
            }
        }
    ...
    #applySetAck(op) {
        ...
        if (existingItem !== void 0) {
            ...
            if (orphan && this.#implicitlyDeletedItems.has(orphan)) {
                ...
                if (indexOfItemWithSamePosition !== -1) {
                    const displaced = nn(this.#items.removeAt(indexOfItemWithSamePosition));
   ...
   */ #bumpUnackedPushesAbove(remoteKey) {
    ...
    #applySetUndoRedo(op) {
        ...
        if (indexOfItemWithSameKey !== -1) {
            ...
            this.#items.add(child);
            const reverse = addIntentToRootOp(existingItem._toOps(nn(this._id), key), "set", op.id);
            const delta = [
   ...
   */ #injectAt(element, index, intent) {
        ...
        if (this._pool && this._id) {
            ...
            this._pool.dispatch(intent === "push" ? addIntentToRootOp(ops, "push") : ops, [
                {
                    type: OpCode.DELETE_CRDT,
                    id
                }
   ...
   */ delete(index) {
    ...
    set(index, item) {
        ...
        if (this._pool && this._id) {
            ...
            const ops = addIntentToRootOp(value._toOpsWithOpId(this._id, position, this._pool), "set", existingId);
            const reverseOps = addIntentToRootOp(existingItem._toOps(this._id, position), "set", id);
            this._pool.dispatch(ops, reverseOps, storageUpdates);
        }
    }
    #unwrap(node) {
   ...
   */ indexOf(searchElement, fromIndex) {
        return this.#items.rawArray.findIndex((node, i)=>i >= (fromIndex ?? 0) && this.#unwrap(node) === searchElement);
    }
    /**
...
var LiveObject = class _LiveObject extends AbstractCrdt {
   ...
   */ update(patch) {
        ...
        if (_LiveObject.detectLargeObjects) {
            ...
            if (upperBoundSize > MAX_LIVE_OBJECT_SIZE) {
                ...
                if (preciseSize > MAX_LIVE_OBJECT_SIZE) {
                    throw new Error(`LiveObject size exceeded limit: ${preciseSize} bytes > ${MAX_LIVE_OBJECT_SIZE} bytes. See https://liveblocks.io/docs/pricing/limits#Other-limits`);
                }
            }
        }
        if (this._pool === void 0 || this._id === void 0) {
    ...
    reconcile(jsonObj, config) {
        ...
        if (!isPlainObject(jsonObj)) throw new Error("Reconciling the document root expects a plain object value");
   ...
   */ reconcilePartially(partialObj, config) {
        if (!isPlainObject(partialObj)) throw new Error("Reconciling the document root expects a plain object value");
    ...
    /** @internal */ _toTreeNode(key) {
        ...
        return {
            ...
            id: nodeId,
            key,
            payload: Array.from(this.#synced.entries()).map(([key2, value])=>isLiveNode(value) ? value.toTreeNode(key2) : {
                    type: "Json",
                    id: `${nodeId}:${key2}`,
                    key: key2,
                    payload: value
    ...
    clone() {
        const cloned = new _LiveObject(Object.fromEntries(Array.from(this.#synced).map(([key, value])=>[
                key,
...
function liveObjectFromNodeStream(nodes) {
    const pool = createManagedPool({
        getCurrentConnectionId: ()=>{
            throw new Error("Cannot mutate a historic storage version: it is a read-only snapshot");
        }
    });
...
function createRoom(options, config) {
    ...
    async function listHistoryVersions() {
        return httpClient.listHistoryVersions({
            roomId
        });
    }
    ...
    async function fetchStorageHistoryVersion(versionId) {
        return httpClient.fetchStorageHistoryVersion({
            roomId,
            versionId
        });
    }
    async function fetchYjsHistoryVersion(versionId) {
        return httpClient.fetchYjsHistoryVersion({
            roomId,
            versionId
        });
    }
    async function createVersionHistorySnapshot() {
        return httpClient.createVersionHistorySnapshot({
            roomId
        });
    }
    async function deleteHistoryVersion(versionId) {
        return httpClient.deleteHistoryVersion({
            roomId,
            versionId
        });
    }
    ...
    const self = DerivedSignal.from(context.staticSessionInfoSig, context.dynamicSessionInfoSig, context.myPresence, (staticSession, dynamicSession, myPresence)=>{
        if (staticSession === null || dynamicSession === null) {
            ...
            const canWrite = hasPermissionAccess(dynamicSession.permissionMatrix, "storage", "write");
            return {
                ...
                presence: myPresence,
                canWrite,
                canComment: hasPermissionAccess(dynamicSession.permissionMatrix, "comments", "write")
            };
        }
    });
    let _lastSelf;
    ...
    function reconcileStorageWithNodes(nodes) {
        ...
        const ops = diffCurrentStorageAgainst(new Map(nodes));
        if (ops.length === 0) {
            return;
        }
    ...
    function onRoomStateMessage(message) {
        context.dynamicSessionInfoSig.set({
            actor: message.actor,
            nonce: message.nonce,
            permissionMatrix: permissionMatrixFromScopes(normalizeRoomPermissions(message.scopes)),
            meta: message.meta
        });
        ...
        for(const key in message.users){
            ...
            context.others.setConnection(connectionId, user.id, user.info, connectionAccessFromScopes(user.scopes));
        }
        return {
            type: "reset"
        };
    ...
    function onUserJoinedMessage(message) {
        context.others.setConnection(message.actor, message.id, message.info, connectionAccessFromScopes(message.scopes));
                ...
                case ServerMsgCode.FEED_MESSAGES_LIST:
                    {
                        ...
                        const pending = pendingFeedMessagesRequests.get(feedMsgsListMsg.requestId);
                        if (pending) {
                            pending.resolve({
    ...
    function registerFeedMutation(requestId, kind, feedId, options2) {
        ...
        const timeoutId = setTimeout(()=>{
            if (pendingFeedMutations.has(requestId)) {
                settleFeedMutation(requestId, "error", new Error("Feed mutation timeout"));
            }
        }, FEEDS_TIMEOUT);
    ...
    function processInitialStorage(nodes) {
        const unacknowledgedOps2 = [
            ...context.unacknowledgedOps.values()
        ];
    ...
    async function fetchFeeds(options2) {
        ...
        const { promise, resolve, reject } = Promise_withResolvers();
        pendingFeedsRequests.set(requestId, {
            resolve,
            reject
        });
        const message = {
    ...
    async function fetchFeedMessages(feedId, options2) {
        const requestId = nanoid();
        const { promise, resolve, reject } = Promise_withResolvers();
        pendingFeedMessagesRequests.set(requestId, {
            resolve,
            reject
        });
        const message = {
    ...
    function uploadFile(file, options2 = {}) {
    ...
    function getFileUrl(file) {
        return httpClient.getFileUrl({
            roomId,
            fileId: getLiveFileId(file)
        });
    }
    ...
    return Object.defineProperty({
        [kInternal]: {
            ...
            simulate: {
                ...
                incomingMessage: (data)=>handleServerMessage(new MessageEvent("message", {
                        data
                    }))
            },
        ...
        id: roomId,
        subscribe: makeClassicSubscribeFn(roomId, eventHub, config.errorEventSource),
...
${dumpPool(context.pool)}`;
        ...
        destroy: ()=>{
            pendingFeedsRequests.forEach((request)=>request.reject(new Error("Room destroyed")));
            pendingFeedMessagesRequests.forEach((request)=>request.reject(new Error("Room destroyed")));
...
function createClient(options) {
    ...
    const throttleDelay = ("TURBOPACK compile-time value", "development") !== "production" && isLocalhost(baseUrl) && clientOptions.__DANGEROUSLY_disableThrottling ? 0 : getThrottle(clientOptions.throttle ?? DEFAULT_THROTTLE);
...
//# sourceMappingURL=node_modules_%40liveblocks_core_dist_index_13wwvtc.js.map